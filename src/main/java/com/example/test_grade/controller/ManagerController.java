...
import com.example.test_grade.facade.UserServiceFacade;
...
public class ManagerController {
    ...
    public Result update(@RequestBody Manager manager) {
        return userServiceFacade.updateManager(manager);
    }

    @PostMapping
    public Result add(@RequestBody Manager manager) {
        return userServiceFacade.addManager(manager);
    }

    @GetMapping
    public Result getAllManagers() {
        return userServiceFacade.getAllManagers();
    }

    @DeleteMapping("/{id}")
    public Result deleteById(@PathVariable Long id) {
        return userServiceFacade.deleteManager(id);
    }

    @GetMapping("/{name}")
    public Result findManagerByName(@PathVariable String name) {
        return userServiceFacade.findManagersByName(name);
    }
